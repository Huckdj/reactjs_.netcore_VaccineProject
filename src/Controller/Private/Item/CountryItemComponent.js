...
function CountryItemComponent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post(`${urlapi}/api/CountryItem/GetData`);
  ...
  setIsLoading(!isLoading)
    if(payload.NameCountry === ""){
        setIsLoading(!isLoading)
        setResmsg("Không bỏ trống Tên nước")
            if (msg === false) {
                setMsg(true);
    ...
    if(payload.ID === "" && payload.NameCountry !== ""){
        axios.post(`${urlapi}/api/CountryItem/InsertCountry`,payload)
        .then(res=>{
  ...
  const hangleDelete = (e) =>{
    setIsLoading(true)
    const id = e
    axios.post(`${urlapi}/api/CountryItem/DeletCountry`,{id})
    .then(res=>{
        setResmsg(res.data.data[0].ErrorMessage)
        setIsLoading(false)
        if (msg === false) {
  ...
  const handleSaveEdit = (e) =>{
    setIsLoading(true)
    axios.post(`${urlapi}/api/CountryItem/EditById`,payload)
    .then(res=>{
        setResmsg(res.data.data[0].ErrorMessage)
        setOpenEdit(false)
        setReloaddata(true)
  ...
  const handleSetActive = (e) =>{
    ...
    axios.post(`${urlapi}/api/CountryItem/SetActive`,updatedPayload)
      .then(res=>{
        setResmsg(res.data.data[0].ErrorMessage)
        setReloaddata(true)
        setIsLoading(false)
    ...
    <div>
      <div className=" my-6 overflow-x-auto mx-auto border rounded-lg shadow-lg ml-2 mr-2">
        ...
        >
          ...
          <div>
            <div className="m-2 font-semibold">Tên nước</div>
            <input
              id="insertNameCountry"
              className="border p-1 outline-none m-2 w-[300px] placeholder:font-semibold font-semibold focus:outline-blue-500"
              placeholder="Nhập tên nước sản xuất"
              onChange={(e) => setPayload({...payload,NameCountry:e.target.value})}
              value={payload.NameCountry}
            />
          </div>
        </div>
